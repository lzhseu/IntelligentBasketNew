...
class PhoneField: UITextField {
    ...
    //保持上一次的文本范围
    private var _previousRange: UITextRange!
    ...
    @objc func phoneNumberFormat(_ notification: Notification) {
        ...
        //输入的第一个数字必需为1
        if textField.text != "" && Int(textField.text![0]) != 1 {
        ...
        //得到带有分隔符的字符串
        let hyphenText = getHyphenText(string: digitsText, cursorPosition: &cursorPostion, isAddSeparation: false)
    ...
    func getHyphenText(string:String, cursorPosition:inout Int, isAddSeparation: Bool) -> String {
        //保存开始时光标的位置
        let originalCursorPosition = cursorPosition
        //处理后的结果字符串
        var result = ""
